...
| 12.4 | **Slot 06 da Comissão Técnica também estava com coordenada obsoleta** (mesma causa da 12.3 — arte reexportada). Comparando o print vazio com o slot 07 (confirmado correto): 06 e 07 estão emparelhados, mesma altura, mesmo topo — mas os dados tinham 06 mais baixo e mais curto (`y=261,altura=345` vs `y=214,altura=392` do 07). Corrigido para `y=214,altura=392`, igual ao 07 | cc:完了 |

---

## Fase 13: Scroll horizontal do inventário + troca de repetidas (2026-07-05)

| Task | Conteúdo | Status |
|------|------|--------|
| 13.1 | **Scroll horizontal do inventário**: o container já tinha `overflow-x-auto`, mas os cards são `draggable="true"` (drag nativo pra colar no álbum) — isso sequestra o gesto de clique-e-arraste do mouse, então não dava pra rolar arrastando com o mouse, e a barra de rolagem estava propositalmente escondida (`hide-scrollbar`), deixando sem nenhuma forma óbvia de rolar quando há muitas figurinhas. Adicionadas 2 setas (‹ ›) fixas nas bordas da barra que chamam `container.scrollBy()` — funciona independente do conflito com o drag nativo. Setas escondidas em mobile (`hidden md:flex`) porque touch-scroll (swipe) já funciona nativamente lá | cc:完了 |
| 13.2 | **Detecção de repetida corrigida**: o selo existente (renomeado de "DUP" pra "REPETIDA") só considerava repetida uma figurinha já COLADA em algum slot — duas cópias da MESMA figurinha no inventário, nenhuma ainda colada, não eram marcadas. Nova função `obterIndicesRepetidos()` marca como repetida toda cópia que já foi colada OU que não é a 1ª ocorrência daquele número dentro do inventário (a 1ª cópia continua "precisando ser colada", só a 2ª em diante é sobra). Clicar num card repetido mostra um toast confirmando (além do selo permanente já visível) | cc:完了 |
| 13.3 | **Botão "Trocar Repetidas"**: aparece (some por padrão) quando `obterIndicesRepetidos().length >= 5`. Ao clicar (com confirmação), remove TODAS as cópias repetidas do inventário (não só 5) e sorteia a mesma quantidade de números aleatórios do pool global, reaproveitando o modal de revelação de cartas já existente (mesma experiência visual de "abrir pacote", sem consumir pacote nem mostrar a animação de rasgar embrulho). Refatorado `sortearNumeros(tamanho)` como helper compartilhado entre `rasgarPacote()` e `trocarRepetidas()` (mesmo algoritmo de sorteio sem repetição dentro do lote, só extraído pra função separada — comportamento de `rasgarPacote()` não mudou) | cc:完了 |

> Nenhuma funcionalidade existente foi alterada: drag-and-drop pra colar, cliques em figurinha colada (devolver ao inventário), abertura de pacote normal e o layout dos slots continuam exatamente como estavam. Requer recolar `Album.html` no Apps Script e republicar.
